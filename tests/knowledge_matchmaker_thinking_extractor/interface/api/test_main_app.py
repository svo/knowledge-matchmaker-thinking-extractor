...
def test_should_create_openapi_json_file():
    ...
    from knowledge_matchmaker_thinking_extractor.interface.api.main import app as rest
...
class TestMainApp:
    def test_should_have_application_title(self):
        assert_that(app.title).is_equal_to("Knowledge Matchmaker Thinking Extractor API")
    ...
    @patch("uvicorn.run")
    @patch("knowledge_matchmaker_thinking_extractor.interface.api.main.get_application_setting_provider")
    def test_main_function(self, mock_get_provider, mock_run):
        from knowledge_matchmaker_thinking_extractor.shared.configuration import ApplicationSettingProvider
    ...
    @patch("knowledge_matchmaker_thinking_extractor.interface.api.main.main")
